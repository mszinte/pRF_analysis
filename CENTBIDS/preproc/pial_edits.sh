...
# 1. cd to function
# >> cd ~/projects/pRF_analysis/CENTBIDS/preproc
# 2. run shell command
# sh pial_edits.sh [main directory] [project name] [subject name] [mesocentre_ID]
# -----------------------------------------------------------------------------------------
# Exemple:
# sh pial_edits.sh /scratch/jstellmann/data CENTBIDS sub-2100247523 skling
# -----------------------------------------------------------------------------------------
...
rsync -azuv --rsh='ssh -p 8822' --progress $4@login.mesocentre.univ-amu.fr:$1/$2/derivatives/freesurfer/$3 ~/temp_data/
...
# removed ~/temp_data/$3/mri/T2.mgz / because no T2_w images 
...
while true; do
    ...
    case $yn in
        [Yy]* ) echo "\n>> Uploading of the brainmasks to mesocentre";\
                rsync -azuv --rsh='ssh -p 8822' ~/temp_data/$3/mri/brainmask.mgz $4@login.mesocentre.univ-amu.fr:$1/$2/derivatives/freesurfer/$3/mri/
                rsync -azuv --rsh='ssh -p 8822' ~/temp_data/$3/mri/brainmask_orig.mgz $4@login.mesocentre.univ-amu.fr:$1/$2/derivatives/freesurfer/$3/mri/
        break;;
