"""
-----------------------------------------------------------------------------------------
make_inter_task_rois_fig_tsv.py
-----------------------------------------------------------------------------------------
Goal of the script:
...
To run:
1. cd to function
>> cd ~/projects/[PROJECT]/analysis_code/postproc/inter_task_analysis/
...
python make_inter_task_rois_fig_tsv.py /scratch/mszinte/data RetinoMaps sub-01 327
python make_inter_task_rois_fig_tsv.py /scratch/mszinte/data RetinoMaps sub-170k 327
python make_inter_task_rois_fig_tsv.py /scratch/mszinte/data RetinoMaps group 327
-----------------------------------------------------------------------------------------
...
for format_, extension in zip(formats, extensions):
    ...
    if 'group' not in subject:
        ...
        for categorie_to_plot in categories_to_plot : 
            # data_raw_categorie = data_raw.loc[data_raw['all'] == categorie_to_plot]
            data_categorie = data.loc[data['all'] == categorie_to_plot]

            # # ROI surface areas 
            # # -----------------
            # data_raw_categorie['vert_area'] = data_raw_categorie['vert_area'] / 100 # in cm2
            # df_roi_area = data_raw_categorie.groupby(['roi'], sort=False)['vert_area'].sum().reset_index()
            ...
            os.makedirs(tsv_dir_categorie, exist_ok=True)
