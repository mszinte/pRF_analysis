...
Exemple:
cd ~/projects/pRF_analysis/RetinoMaps/intertask/
python make_active_vert_tsv.py /scratch/mszinte/data RetinoMaps sub-01 327
python make_active_vert_tsv.py /scratch/mszinte/data RetinoMaps sub-170k 327
python make_active_vert_tsv.py /scratch/mszinte/data RetinoMaps group 327
-----------------------------------------------------------------------------------------
    ...
    for format_ in formats :
        ...
        if 'group' not in subject:
            ...
            for roi in rois : 
                ...
                n_vert_roi_vision_and_pursuit_and_saccade = data.loc[(data['roi'] == roi) & (data['saccade'] == 'saccade') & (data['pursuit'] == 'pursuit') & (data['vision'] == 'vision'), 'vert_area'].shape[0]
        ...
        if format_ == '170k':
            ...
            for i, subject_to_group in enumerate(subjects_to_group):
                ...
                # Active Vertex roi
                # -----------------
                tsv_active_vertex_roi_fn = "{}/{}_active_vertex_roi_{}.tsv".format(intertask_tsv_dir, subjects_to_group, suffix)
                ...
                # Active Vertex mmp roi
                # ---------------------
                tsv_active_vertex_roi_mmp_fn = "{}/{}_active_vertex_roi_mmp_{}.tsv".format(intertask_tsv_dir, subjects_to_group, suffix)
            ...
            # Active Vertex roi
            # -----------------
            df_active_vertex_roi_median_df = df_active_vertex_roi.groupby(['roi','categorie'], sort=False).median(numeric_only=True).reset_index()
            df_active_vertex_roi_ci_low_df = df_active_vertex_roi.groupby(['roi', 'categorie'], sort=False).quantile(0.025, numeric_only=True).reset_index()
            df_active_vertex_roi_ci_high_df = df_active_vertex_roi.groupby(['roi', 'categorie'], sort=False).quantile(0.975, numeric_only=True).reset_index()
            
            group_active_vertex_roi_df = pd.concat([df_active_vertex_roi_median_df, 
            ...
            # Active Vertex MMP roi
            # ---------------------
            df_active_vertex_mmp_roi_median_df = df_active_vertex_roi.groupby(['roi','roi_mmp', 'categorie'], sort=False).median(numeric_only=True).reset_index()
            df_active_vertex_mmp_roi_ci_low_df = df_active_vertex_roi.groupby(['roi','roi_mmp', 'categorie'], sort=False).quantile(0.025, numeric_only=True).reset_index()
            df_active_vertex_mmp_roi_ci_high_df = df_active_vertex_roi.groupby(['roi','roi_mmp', 'categorie'], sort=False).quantile(0.975, numeric_only=True).reset_index()
            
            group_active_vertex_mmproi_df = pd.concat([df_active_vertex_mmp_roi_median_df, 
            ...
            tsv_active_vertex_roi_mmp_fn = "{}/{}_active_vertex_roi_mmp_{}.tsv".format(intertask_tsv_dir, subject, suffix)
            print('Saving tsv: {}'.format(tsv_active_vertex_roi_mmp_fn))
            group_active_vertex_mmproi_df.to_csv(tsv_active_vertex_roi_mmp_fn, sep="\t", na_rep='NaN', index=False)
