...
To run:
1. cd to function
>> cd ~/projects/pRF_analysis/RetinoMaps/glm/postfit/dev
2. run python command
>> python compute_glm_stats.py [main directory] [project name] [subject num] [group]
-----------------------------------------------------------------------------------------
Exemple:
cd ~/projects/pRF_analysis/RetinoMaps/glm/postfit/dev
python compute_glmsingle_stats.py /scratch/mszinte/data RetinoMaps sub-01 327
python compute_glmsingle_stats.py /scratch/mszinte/data RetinoMaps sub-170k 327
-----------------------------------------------------------------------------------------
...
if subject != 'sub-170k':
    for format_, extension in zip(formats, extensions): 
        ...
        for task in tasks:
            # Find glm fit files 
            glm_fit_dir = '{}/{}/derivatives/pp_data/{}/{}/glm/glm_single_fit'.format(
            ...
            for glm_pred_loo_fn in glm_pred_loo_fns_list : 
                ...
                # Save results 
                glm_deriv_dir = '{}/{}/derivatives/pp_data/{}/{}/glm/glm_single_derivatives'.format(
                    main_dir, project_dir, subject, format_)
    ...
    for format_, extension in zip(formats, extensions):
        list_ = glob.glob("{}/{}/derivatives/pp_data/{}/{}/glm/glm_single_derivatives/*loo-*_glm-stats.{}".format(
    ...
    for loo_stats_fns in loo_stats_fns_list:
        for task in tasks:
            ...
            if hemi:
                median_fn = '{}/{}/derivatives/pp_data/{}/fsnative/glm/glm_single_derivatives/{}'.format(
                    main_dir, project_dir, subject, loo_stats_median_fn)
                hemi_data_median[hemi] = loo_stats_data_median
            else:
                median_fn = '{}/{}/derivatives/pp_data/{}/170k/glm/glm_single_derivatives/{}'.format(
                    main_dir, project_dir, subject, loo_stats_median_fn)
...
# Sub-170k median
    ...
    print('sub-170, computing median glm stats across subject...')
    
    for task in tasks:
        # find all the subject prf derivatives
        subjects_stats_task = []
        for subject in subjects:
            subjects_stats_task += ["{}/{}/derivatives/pp_data/{}/170k/glm/glm_single_derivatives/{}_task-{}_fmriprep_dct_avg_glm-stats_loo-median.dtseries.nii".format(
                    main_dir, project_dir, subject, subject, task)]
    
        # Computing median across subject
        img, data_stat_median = median_subject_template(fns=subjects_stats_task)
        ...
        # Export results
        sub_170k_stats_dir = "{}/{}/derivatives/pp_data/sub-170k/170k/glm/glm_single_derivatives/".format(
