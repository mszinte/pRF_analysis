...
# Define subjects, ROIs, hemispheres, data types
  ...
  for HEMI in "${HEMIS[@]}"; do
    for TYPE in "${TYPES[@]}"; do
      ...
      if [[ "$TYPE" == "full_corr" ]]; then
        SUBFOLDER="full_corr"
        DESC_PREFIX="desc-full_corr"
      elif [[ "$TYPE" == "fisher-z" ]]; then
        SUBFOLDER="fisher-z"
        DESC_PREFIX="desc-fisher-z"
      fi

      # Build filename descriptor: e.g., desc-full_corr_lh_mPCS
      if [[ "$HEMI" == "both" ]]; then
        DESC="${DESC_PREFIX}_${ROI}"
      else
        DESC="${DESC_PREFIX}_${HEMI}_${ROI}"
      fi
      ...
      echo "🔄 Merging TYPE=${TYPE} | ROI=${ROI} | HEMI=${HEMI}..."
      "${MERGE_CMD[@]}"
