...
GROUP_DIR="$TASK_RESULTS/group/91k/rest/full_corr"
...
mkdir "$TASK_RESULTS/sub-${i}/91k/rest/corr/full_corr"
    ...
    do
        ...
        wb_command -cifti-correlation "$TASK_RESULTS/sub-${i}/91k/rest/timeseries/sub-${i}_ses-01_task-rest_space-fsLR_den-91k_desc-denoised_bold.dtseries.nii" \
            "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}.dconn.nii" -roi-override \
        ...
        wb_command -cifti-correlation "$TASK_RESULTS/sub-${i}/91k/rest/timeseries/sub-${i}_task-rest_space-fsLR_den-91k_desc-denoised_bold.dtseries.nii" \
        ...
        wb_command -cifti-average-dense-roi "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}.dscalar.nii" \
            -left-roi "$SEED_DIR/sub-${i}_91k_intertask_Sac_Pur_vision-pursuit-saccade_lh_${ROI}.shape.gii" \
            -right-roi "$SEED_DIR/sub-${i}_91k_intertask_Sac_Pur_vision-pursuit-saccade_rh_${ROI}.shape.gii" \
            -cifti "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}.dconn.nii";
        ...
        wb_command -cifti-average-dense-roi "$OUT_DIR/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}.dscalar.nii" \
            -left-roi "$SEED_DIR/sub-${i}_91k_intertask_Sac_Pur_vision-pursuit-saccade_lh_${ROI}.shape.gii" \
            -right-roi "$SEED_DIR/sub-${i}_91k_intertask_Sac_Pur_vision-pursuit-saccade_rh_${ROI}.shape.gii" \
            -cifti "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}.dconn.nii";
            
        # Average correlation values within the target ROIs (both hemispheres)
        wb_command -cifti-parcellate "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}.dscalar.nii" \
       ...
       # Same step as previous with Fisher-z outputs
        ...
        "$ATLAS/atlas-Glasser_space-fsLR_den-32k_filtered_ROIs_dseg.dlabel.nii" COLUMN \
        "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}_parcellated.pscalar.nii" -method MEAN;
        
        # Average correlation values within the target ROIs (both hemispheres) this time excluding outliers -/+3 SD above the mean
        wb_command -cifti-parcellate "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}.dscalar.nii" \
        "$ATLAS/atlas-Glasser_space-fsLR_den-32k_filtered_ROIs_dseg.dlabel.nii" COLUMN \
        "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}_parcellated_no_outliers.pscalar.nii" -method MEAN \
        -exclude-outliers 3 3;
        
       # Same step as previous with Fisher-z outputs
        wb_command -cifti-parcellate "$OUT_DIR/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}.dscalar.nii" \
        "$ATLAS/atlas-Glasser_space-fsLR_den-32k_filtered_ROIs_dseg.dlabel.nii" COLUMN \
        "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}_parcellated_no_outliers.pscalar.nii" -method MEAN \
        -exclude-outliers 3 3;

        # Mask vertex-wise results for visualizations in supplementary information
        wb_command -cifti-restrict-dense-map "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}.dscalar.nii" COLUMN \
            "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}_masked.dscalar.nii" \
        ...
        wb_command -cifti-restrict-dense-map "$OUT_DIR/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}.dscalar.nii" COLUMN \
            "$OUT_DIR/full_corr/sub-${i}_task-rest_space-fsLR_den-91k_desc-fisher-z_${ROI}_masked.dscalar.nii" \
            -left-roi "$ATLAS/atlas-Glasser_space-fsLR_den-32k_filtered_ROIs_left_hemi.shape.gii" \
            -right-roi "$ATLAS/atlas-Glasser_space-fsLR_den-32k_filtered_ROIs_right_hemi.shape.gii";
