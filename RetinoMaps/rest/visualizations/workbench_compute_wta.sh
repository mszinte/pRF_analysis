...
for sub in 01 02 03 04 05 06 07 08 09 11 12 13 14 17 20 21 22 23 24 25; do
    ...
    # Remove volume info so we end up with approximately 59k vertices
    for roi in "${ROIS[@]}"; do
    	...
    	COLUMN \
    	-left-roi "$ATLAS_DIR/atlas-Glasser_space-fsLR_den-32k_left_hemi.shape.gii" \
    	-right-roi "$ATLAS_DIR/atlas-Glasser_space-fsLR_den-32k_right_hemi.shape.gii" \
    ...
    # Zero out values outside the atlas
    	...
    	cp "${FULL_CORR}/sub-${sub}_task-rest_space-fsLR_den-91k_desc-full_corr_${roi}_cortex.dscalar.nii" "${FULL_CORR}/sub-${sub}_task-rest_space-fsLR_den-91k_desc-full_corr_${roi}_cortex_b.dscalar.nii"
