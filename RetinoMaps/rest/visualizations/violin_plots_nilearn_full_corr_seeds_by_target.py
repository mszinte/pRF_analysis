...
for sub in subjects:
    ...
    sub_path = os.path.join(main_data,
                            f"{sub}/91k/rest/corr/full_corr/workbench_full_corr/by_hemi")
    for seed in seed_clusters:
        for hemi in hemis:
            ...
            cluster_full = pd.read_csv(fname, header=None, sep='\t').squeeze().to_numpy(dtype=float)

            print(f"{sub} ({hemi}): {cluster_full.shape}")

            seed_idx = clusters.index(seed)

            values = cluster_full[:53] if hemi == "lh" else cluster_full[53:]
...
for target in target_clusters:
    ...
    outname = os.path.join(
        fig_dir,
        f"violin_target-{target}_full_corr_by_hemi_fisher-z_seeds_by_target.png"
    )
