...
wb_command -cifti-reduce "${BASE_DIR}/median_full_corr/group_ses-01_task-rest_space-fsLR_den-91k_desc-full_corr_${ROI}_median.dscalar.nii" \
...
wb_command -cifti-math 'corr * (seed == 0) + (-0.999 * (seed > 0))' \
    ...
    -var seed "${SEED_DIR}/atlas-Glasser_space-fsLR_den-32k_filtered_ROIs_leaveout_${ROI}.dlabel.nii" \
    -select 2 INDEXMAX;
