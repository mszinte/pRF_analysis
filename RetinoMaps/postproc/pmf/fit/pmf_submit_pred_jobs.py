...
def get_prf_avg_gauss_fit_fn(pp_dir, subject, pp_fn, preproc_prep, filtering, normalization):
    ...
    if pp_fn.endswith('.nii'):
        pattern = "{}/{}/170k/prf/fit/{}_task-{}_{}_{}_{}_avg_prf-gauss_fit*.nii".format(
            pp_dir, subject, subject, prf_task_names[0], preproc_prep, filtering, normalization)
        ...
        pattern = "{}/{}/fsnative/prf/fit/{}_task-{}_{}_{}_{}_{}_avg_prf-gauss_fit*.gii".format(
            pp_dir, subject, subject, prf_task_names[0], hemi, preproc_prep, filtering, normalization)
        matches = glob.glob(pattern)
...
for subject in subjects:
    ...
    for avg_method in avg_methods:
        ...
        dct_avg_gii_fns = "{}/{}/fsnative/func/{}_{}_{}_{}/*_task-{}*{}*.func.gii".format(
            pp_dir, subject, preproc_prep, filtering, normalization, avg_method, pmf_task_names[0], avg_method)
    ...
    if not pp_fns:
        print("WARNING: No preprocessed files found for subject {}, skipping.".format(subject))
        continue
    ...
    for fit_num, pp_fn in enumerate(pp_fns):
        ...
        if pp_fn.endswith('.nii'):
            ...
            pmf_logs_dir = "{}/{}/170k/pmf/log_outputs".format(pp_dir, subject)
        ...
        prf_gauss_fit_fn = get_prf_avg_gauss_fit_fn(
            pp_dir, subject, pp_fn, preproc_prep, filtering, normalization)
...
""".format(server_project=server_project, cluster_name=cluster_name,
        ...
        # define fit cmd
        fit_cmd = "python pmf_pred_from_prf.py {} {} {} {} {} {}".format(
            main_dir, project_dir, subject, pp_fn, prf_gauss_fit_fn, nb_procs)
