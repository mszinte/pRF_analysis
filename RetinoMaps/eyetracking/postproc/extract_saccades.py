...
for task in tasks :
    ...
    runs = np.arange(0, task_settings['num_run'], 1)
    sequences = np.arange(0, task_settings['num_seq'], 1)
    trials_seq = task_settings['trials_seq']
    rads = task_settings['rads']
    polar_ang = np.deg2rad(np.arange(0, 360, task_settings['ang_steps']))
    pursuits_tr = np.arange(0, task_settings['seq_trs'] ,2)
    saccades_tr = np.arange(1, task_settings['seq_trs'], 2)
    seq_type = task_settings['seq_type']
    ...
    # Get saccade model
    sampling_rate = task_settings['eyetrack_sampling']
    velocity_th = task_settings['velocity_th']
    min_dur = task_settings['min_dur']
    merge_interval = task_settings['merge_interval']
    tolerance_ratio = task_settings['tolerance_ratio']
