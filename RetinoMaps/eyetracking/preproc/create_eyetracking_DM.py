...
python create_eyetracking_DM.py /scratch/mszinte/data RetinoMaps sub-01 327
-----------------------------------------------------------------------------------------
...
def create_design_matrix(event_file, json_file):
    ...
    event_description.pop('StimulusPresentation', None)
...
for task in tasks : 
    ...
    task_settings = load_settings([task_settings_path])[0] 

    
    # Define file list
    num_run = task_settings.get('num_run')
