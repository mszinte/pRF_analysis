        ...
        for prf_task_name in prf_task_names:
            ...
            if 'group' not in subject:
                ...
                for num_roi, roi in enumerate(rois):
                    ...
                    df_params_median_roi['pcm_median_weighted_median'] = weighted_nan_median(df_roi.pcm_median, weights=df_roi[rsq2use])
                    df_params_median_roi['pcm_median_ci_down'] = weighted_nan_percentile(df_roi.pcm_median, df_roi[rsq2use], 25)
                    df_params_median_roi['pcm_median_ci_up'] = weighted_nan_percentile(df_roi.pcm_median, df_roi[rsq2use], 75)
                ...
                for num_roi, roi in enumerate(rois):
                    ...
                    df_ecc_pcm_bin['prf_pcm_bins_ci_upper_bound'] = df_bins.apply(lambda x: weighted_nan_percentile(x['pcm_median'].values, x[rsq2use].values, 75)).values
                    df_ecc_pcm_bin['prf_pcm_bins_ci_lower_bound'] = df_bins.apply(lambda x: weighted_nan_percentile(x['pcm_median'].values, x[rsq2use].values, 25)).values
