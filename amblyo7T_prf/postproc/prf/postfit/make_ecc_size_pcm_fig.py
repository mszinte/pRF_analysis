"""
-----------------------------------------------------------------------------------------
make_ecc_size_pcm_fig.py
-----------------------------------------------------------------------------------------
Goal of the script:
Make per-subject or per-group figures of eccentricity vs pRF size (row 1),
...
-----------------------------------------------------------------------------------------
Output(s):
- Per-subject/group, per-task ecc-size-pcm figure (PDF):
  {subject}_{fn_spec_combined}_ecc-size-pcm.pdf
-----------------------------------------------------------------------------------------
To run:
1. cd to function
>> cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit/
2. run python command
python make_ecc_size_pcm_fig.py [main directory] [project name] [subject] [group]
-----------------------------------------------------------------------------------------
Example:
cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit/
...
from plot_utils import eyes_ecc_size_pcm_plot
        ...
        for rois_method_format in rois_methods_format:
            ...
            for prf_task_eyes_names in prf_tasks_eyes_names:
                ...
                if not os.path.isfile(tsv_fn):
                    print(f"[SKIP] TSV not found: {tsv_fn}")
                    continue
