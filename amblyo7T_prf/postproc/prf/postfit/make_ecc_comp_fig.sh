#!/bin/bash
#-----------------------------------------------------------------------------------------
# make_ecc_comp_fig.sh
#-----------------------------------------------------------------------------------------
# Goal of the script:
# Launch across subjects the function make_ecc_comp_fig.py
#-----------------------------------------------------------------------------------------
# Input(s):
...
#-----------------------------------------------------------------------------------------
# Output(s):
# Eccentricity comparison figures per eye condition per subject and group
#-----------------------------------------------------------------------------------------
# To run:
# 1. cd to function
...
# Example:
# cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
# sh make_ecc_comp_fig.sh /home/mszinte/projects amblyo7T_prf /scratch/mszinte/data 327
#-----------------------------------------------------------------------------------------
...
do
    echo "Processing make_ecc_comp_fig.py for: $subject"
    python make_ecc_comp_fig.py "$data_path" "$project_name" "$subject" "$group"
done

# Run for groups
for group_label in group-patient group-control
do
    echo "Processing make_ecc_comp_fig.py for: $group_label"
    python make_ecc_comp_fig.py "$data_path" "$project_name" "$group_label" "$group"
done