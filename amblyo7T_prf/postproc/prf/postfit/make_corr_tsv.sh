...
#-----------------------------------------------------------------------------------------
# Goal of the script:
# Launch across subjects the function make_corr_tsv.py
#-----------------------------------------------------------------------------------------
# Input(s):
...
#-----------------------------------------------------------------------------------------
# Output(s):
# Correlation TSVs across eyes per subject and group
#-----------------------------------------------------------------------------------------
# To run:
# 1. cd to function
# >> cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
# 2. run shell command
# >> sh make_corr_tsv.sh [code directory] [project name] [main directory] [group]
#-----------------------------------------------------------------------------------------
# Example:
# cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
# sh make_corr_tsv.sh /home/mszinte/projects amblyo7T_prf /scratch/mszinte/data 327
#-----------------------------------------------------------------------------------------
...
do
    echo "Processing make_corr_tsv.py for: $subject"
    python make_corr_tsv.py "$data_path" "$project_name" "$subject" "$group"
done

# Run for groups
for group_label in group-patient group-control
do
    echo "Processing make_corr_tsv.py for: $group_label"
    python make_corr_tsv.py "$data_path" "$project_name" "$group_label" "$group"
done