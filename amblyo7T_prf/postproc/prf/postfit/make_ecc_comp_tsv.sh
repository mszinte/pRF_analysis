#!/bin/bash
#-----------------------------------------------------------------------------------------
# make_ecc_comp_tsv.sh
#-----------------------------------------------------------------------------------------
# Goal of the script:
# Launch across subjects the function make_ecc_comp_tsv.py
#-----------------------------------------------------------------------------------------
# Input(s):
...
#-----------------------------------------------------------------------------------------
# Output(s):
# Eccentricity comparison TSVs per eye condition per subject and group
#-----------------------------------------------------------------------------------------
# To run:
# 1. cd to function
# >> cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
# 2. run shell command
# >> sh make_ecc_comp_tsv.sh [code directory] [project name] [main directory] [group]
#-----------------------------------------------------------------------------------------
# Example:
# cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
...
do
    echo "Processing make_ecc_comp_tsv.py for: $subject"
    python make_ecc_comp_tsv.py "$data_path" "$project_name" "$subject" "$group"
done

# Run for groups (group-combined last as it reloads individual TSVs)
for group_label in group-patient group-control
do
    echo "Processing make_ecc_comp_tsv.py for: $group_label"
    python make_ecc_comp_tsv.py "$data_path" "$project_name" "$group_label" "$group"
done