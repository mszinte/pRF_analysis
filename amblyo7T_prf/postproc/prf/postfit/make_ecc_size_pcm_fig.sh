#!/bin/bash
#-----------------------------------------------------------------------------------------
# make_ecc_size_pcm_fig.sh
#-----------------------------------------------------------------------------------------
# Goal of the script:
# Launch across subjects the function make_ecc_size_pcm_fig.py
#-----------------------------------------------------------------------------------------
# Input(s):
...
#-----------------------------------------------------------------------------------------
# Output(s):
# Eccentricity vs size/pCM figures per eye condition per subject and group
#-----------------------------------------------------------------------------------------
# To run:
# 1. cd to function
# >> cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
# 2. run shell command
# >> sh make_ecc_size_pcm_fig.sh [code directory] [project name] [main directory] [group]
#-----------------------------------------------------------------------------------------
# Example:
# cd ~/projects/pRF_analysis/amblyo7T_prf/postproc/prf/postfit
...
do
    echo "Processing make_ecc_size_pcm_fig.py for: $subject"
    python make_ecc_size_pcm_fig.py "$data_path" "$project_name" "$subject" "$group"
done
# Run for groups
for group_label in group-patient group-control
do
    echo "Processing make_ecc_size_pcm_fig.py for: $group_label"
    python make_ecc_size_pcm_fig.py "$data_path" "$project_name" "$group_label" "$group"
done