...
>> python make_ncsf_tsv.py [main directory] [project name] 
                          [subject num] [group] [analysis folder - optional]
-----------------------------------------------------------------------------------------
Exemple:
cd ~/projects/pRF_analysis/nCSF/postproc/nCSF/postfit/
python make_ncsf_tsv.py /scratch/mszinte/data nCSF sub-01 327
-----------------------------------------------------------------------------------------
...
for avg_method in avg_methods:
    ...
    maps_names = maps_names_ncsf +  ['nCSF_' + x for x in maps_names_ncsf_stats] + maps_names_css + ['pRF_' + x for x in maps_names_css_stats] + maps_names_pcm
    ...
    for format_, extension in zip(formats, extensions):
        ...
        for rois_method_format in rois_methods_format:
            ...
            ncsf_tsv_dir = "{}/tsv".format(ncsf_dir)
            os.makedirs(ncsf_tsv_dir, exist_ok=True)
                
            print(f'{ncsf_task_name} - {avg_method} - {format_}')
            tsv_fn = '{}/{}_task-{}_{}_{}_{}_{}_{}_ncsf_deriv.tsv'.format(
                ncsf_tsv_dir, subject, ncsf_task_name,
            ...
            if format_ == 'fsnative':
                ...
                for hemi in ['hemi-L', 'hemi-R']:
                    ...
                    # nCSF Derivatives
                    ncsf_deriv_fn = '{}/{}_task-{}_{}_{}_{}_{}_{}_ncsf_deriv.func.gii'.format(
                        ncsf_deriv_dir, subject, ncsf_task_name, hemi,
                        preproc_prep, filtering, normalization, avg_method)
                    print(f'loading {ncsf_deriv_fn}')
                    ncsf_deriv_img, ncsf_deriv_mat = load_surface(ncsf_deriv_fn)
                    
                    # nCSF Stats
                    ncsf_stats_fn = '{}/{}_task-{}_{}_{}_{}_{}_{}_ncsf_stats.func.gii'.format(
                        ncsf_deriv_dir, subject, ncsf_task_name, hemi,
                        preproc_prep, filtering, normalization, avg_method)
                    print(f'loading {ncsf_stats_fn}')
                    stats_img, stats_mat = load_surface(ncsf_stats_fn)
                    ...
                    all_deriv_mat = np.concatenate((ncsf_deriv_mat, stats_mat, deriv_mat, stats_mat, pcm_mat))
                ...
                # nCSF Derivatives
                ncsf_deriv_fn = '{}/{}_task-{}_{}_{}_{}_{}_ncsf_deriv.dtseries.nii'.format(
                    ncsf_deriv_dir, subject, ncsf_task_name,
                    preproc_prep, filtering, normalization, avg_method)
                print(f'loading {ncsf_deriv_fn}')
                deriv_img, ncsf_deriv_mat = load_surface(ncsf_deriv_fn)
                
                # nCSF Stats
                ncsf_stats_fn = '{}/{}_task-{}_{}_{}_{}_{}_ncsf_stats.dtseries.nii'.format(
                    ncsf_deriv_dir, subject, ncsf_task_name,
                    preproc_prep, filtering, normalization, avg_method)
                print(f'loading {ncsf_stats_fn}')
                stats_img, stats_mat = load_surface(ncsf_stats_fn)
